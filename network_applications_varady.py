...
class Traceroute(ICMPPing):
    ...
    def sendIcmpProbesAndCollectResponses(self, ttl):
        ...
        for seq_num in range(3):  # Send 3 ICMP probes for each TTL value
            ...
            if seq_num == pkt_keys[-1]:  # Matching based on sequence number
...
class MultiThreadedTraceRoute(Traceroute):
    ...
    def send_probes(self):
        ...
        while ttl <= MAX_TTL:
            ...
            for _ in range(3):
                if args.protocol == "icmp":
                    self.sendIcmpProbesAndCollectResponses(ttl)  # PLACEHOLDER

                elif args.protocol == "udp":
                    pass

                with self.lock:
                    self.sent_probes_.append(f"Sent probe with TTL {ttl}")
    ...
    def receive_responses(self):
        ...
        while not self.send_complete.is_set():
            ...
            if args.protocol == "icmp":
                response = self.receiveOneTraceRouteResponse()  # PLACEHOLDER
                pass
            ...
            if response:
                with self.lock:
                    self.responses_received[response["probe_id"]] = response["data"]
